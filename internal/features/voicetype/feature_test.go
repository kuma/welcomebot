...
func TestName(t *testing.T) {
	f := &Feature{}
	if f.Name() != "voicetype" {
		t.Errorf("expected name 'voicetype', got '%s'", f.Name())
	}
}
...
func TestGetMenuButton(t *testing.T) {
	...
	if button.CustomID != "menu:voicetype:setup" {
		t.Errorf("expected custom ID 'menu:voicetype:setup', got '%s'", button.CustomID)
	}
